...
public class NPCHandler {
  ...
  public static List<CustomNPC> mountBrains(final Chunk chunk) {
    ...
    if (memories != null) {
      for (NPCMemory memory : memories) {
        ...
        if (memory.hasStable()) {
          if (memory.isMainNPC()) {
            npcBrain = CustomNPC.CustomStableNPC(EntityTypes.aV, (World)world, (Location)null);
          } else {
            StableMemory stable = memory.getStable();
            Location origin = new Location(waypoint.getWorld(), stable.getCenterx(), stable.getCentery(), stable.getCenterz());
            npcBrain = CustomNPC.CustomGuardNPC(EntityTypes.aV, (World)world, origin);
          } 
        } else {
          npcBrain = CustomNPC.CustomStableNPC(EntityTypes.aV, (World)world, (Location)null);
        } 
        ...
        (new BukkitRunnable() {
            public void run() {
              final Villager villager = (Villager)Bukkit.getEntity(npcBrain.cm());
              if (villager != null) {
                ...
                (new BukkitRunnable() {
                    public void run() {
                      if (!((List<NPCMemory>)NPCHandler.NPCChunkToLoadedMemory.get(NPCHandler.getChunkKey(chunk))).contains(memory)) {
                        cancel();
                        return;
                      } 
  ...
  public static EntityPlayer createNPCShell(Player p, CustomNPC npcBrain) {
    ...
    EntityPlayer npc = new EntityPlayer((MinecraftServer)dedicatedServer, world, gameProfile);
    Location l = waypoint.getLocation();
    npc.b(l.getX(), l.getY(), l.getZ(), l.getYaw(), l.getPitch());
    NPCShellToBrain.put(npc.cm(), npcBrain);
  ...
  public static void showNPCs(Player p) {
    List<EntityPlayer> npcs = new ArrayList<>((NPCPlayerToHiddenShells.get(p) == null) ? new ArrayList<>() : NPCPlayerToHiddenShells.get(p));
    List<EntityPlayer> toShow = new ArrayList<>();
    List<EntityPlayer> visibleNpcs = new ArrayList<>((NPCPlayerToVisibleShells.get(p) == null) ? new ArrayList<>() : NPCPlayerToVisibleShells.get(p));
    if (npcs != null)
      for (EntityPlayer npc : npcs) {
        CustomNPC brain = NPCShellToBrain.get(npc.cm());
        ...
        if (memory != null) {
          ...
          if (waypoint.getLocation().distance(p.getLocation()) <= 50.0D) {
            ...
            if (textures[0] != null && textures[1] != null) {
              ...
              conn.a((Packet<?>)new PacketPlayOutPlayerInfo(PacketPlayOutPlayerInfo.EnumPlayerInfoAction.a, new EntityPlayer[] { npc }));
              conn.a((Packet<?>)new PacketPlayOutNamedEntitySpawn((EntityHuman)npc));
              conn.a((Packet<?>)new PacketPlayOutEntityTeleport((Entity)npc));
              conn.a((Packet<?>)new PacketPlayOutEntityHeadRotation((Entity)npc, (byte)(int)(l.getYaw() * 256.0F / 360.0F)));
              conn.a((Packet<?>)new PacketPlayOutEntityMetadata(npc.ae(), watcher, true));
              ItemStack item = new ItemStack(Material.DIAMOND_SWORD);
              Pair<EnumItemSlot, net.minecraft.world.item.ItemStack> pair = new Pair<EnumItemSlot, net.minecraft.world.item.ItemStack>(EnumItemSlot.a, CraftItemStack.asNMSCopy(item));
  ...
  public static void hideNPCs(Player p) {
    ...
    for (EntityPlayer npc : NPCPlayerToVisibleShells.get(p)) {
      conn.a((Packet<?>)new PacketPlayOutPlayerInfo(PacketPlayOutPlayerInfo.EnumPlayerInfoAction.e, new EntityPlayer[] { npc }));
    } 
  }
  ...
  public static void showNPC(Player p, EntityPlayer npc) {
    ...
    GameProfile gameProfile = NPCShellToGameProfile.get(npc);
    if (waypoint.getLocation().distance(p.getLocation()) <= 50.0D && 
      textures[0] != null && textures[1] != null) {
      gameProfile.getProperties().put("textures", new Property("textures", textures[0], textures[1]));
      DataWatcher watcher = new DataWatcher((Entity)npc);
      watcher.a(new DataWatcherObject<>(17, DataWatcherRegistry.a), Byte.valueOf((byte) 127));
      PlayerConnection conn = (((CraftPlayer)p).getHandle()).b;
    ...
    if (npcs != null)
      for (EntityPlayer npc : npcs) {
        ...
        if (p == null)
          return; 
        PlayerConnection conn = (((CraftPlayer)p).getHandle()).b;
        conn.a((Packet<?>)new PacketPlayOutEntityHeadRotation((Entity)npc, (byte)(int)(yaw % 360.0F * 255.0F / 360.0F)));
        conn.a((Packet<?>)new PacketPlayOutEntity.PacketPlayOutEntityLook(npc.ae(), (byte)(int)(yaw % 360.0F * 255.0F / 360.0F), (byte)(int)(pitch % 360.0F * 255.0F / 360.0F), true));
      }  
  }
    ...
    if (npcs != null)
      for (EntityPlayer npc : npcs) {
        ...
        if (p == null)
          return; 
        PlayerConnection conn = (((CraftPlayer)p).getHandle()).b;
        conn.a((Packet<?>)new PacketPlayOutEntity.PacketPlayOutRelEntityMoveLook(npc.ae(), (short)(int)(x * 4096.0D), (short)(int)(y * 4096.0D), (short)(int)(z * 4096.0D), (byte)(int)(yaw % 360.0F * 255.0F / 360.0F), (byte)(int)(pitch % 360.0F * 255.0F / 360.0F), true));
        conn.a((Packet<?>)new PacketPlayOutEntityHeadRotation((Entity)npc, (byte)(int)(yaw % 360.0F * 255.0F / 360.0F)));
      }  
  }
  ...
  public static void removeNPC(final CustomNPC brain, Player p, Chunk chunk) {
    ...
    NPCShellToBrain.remove(shell.cm());
    shells.remove(shell);
  ...
  public static void swingMainHand(CustomNPC brain) {
    ...
    if (npcs != null)
      for (EntityPlayer npc : npcs) {
        ...
        p.playSound((org.bukkit.entity.Entity) npc.getBukkitEntity(), Sound.ENTITY_PLAYER_ATTACK_SWEEP, SoundCategory.PLAYERS, 1000.0F, 1.0F);
      }  
  }
}

...
public class PlayerData {
	...
	private Collection<PotionEffectMemory> getPotionFromRace() {
		Collection<PotionEffectMemory> list = new ArrayList<PotionEffectMemory>();
		Player p = Bukkit.getPlayer(getUuid());
		switch (getCurrentRace()) {
		case ZORA:
			if (isUnderwater()) {
				if (getSkyLight() >= 4)
					list.add(new PotionEffectMemory(PotionEffectType.CONDUIT_POWER, 2147483647, 0, false, false, false));
				else
					list.add(new PotionEffectMemory(PotionEffectType.WATER_BREATHING, 2147483647, 0, false, false, false));
			}
			break;
		case GERUDO:
	...
	private void applyEffects(boolean resetEffects) {
		...
		PotionEffectMemory biomeEffect = BiomeRegistry.getRaceFromBiome(currentBiome).contains(currentRace)
				&& currentRace != Race.KOKIRI ? new PotionEffectMemory(PotionEffectType.REGENERATION,2147483647,0,false,false,false) : null;

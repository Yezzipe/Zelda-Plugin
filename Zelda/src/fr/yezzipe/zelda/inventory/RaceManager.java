...
public class RaceManager extends InventoryManager {
  public static void populateRace(Inventory inv) {
  ...
  public static boolean isRaceInventory(Inventory inv) {
    UUID uuid = InventoryUUID.get(inv);
    if (uuid == null)
      return false; 
    if (UUIDTypes.get(uuid) != CustomInventoryType.RACE)
      return false; 
    return true;
  }
  ...
  public static void handleClose(InventoryCloseEvent e) {
    ...
    if (PData.getCurrentRace() == Race.NONE) {
      final Inventory inv = createInventory(null, 9, "Race", CustomInventoryType.RACE);
      populateRace(inv);
      (new BukkitRunnable() {
          public void run() {
            p.openInventory(inv);

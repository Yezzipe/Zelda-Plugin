...
public class ShadowCrystalManager extends InventoryManager {
	...
	if (blockedSlots.contains(Integer.valueOf(e.getRawSlot()))) {
		...
		if (e.getClick() == ClickType.SHIFT_RIGHT && waypoints != null && waypoints.size() > 0) {
		    waypoints.remove(0);
		...
		if (e.getClick() == ClickType.SHIFT_RIGHT && waypoints != null && waypoints.size() > 1) {
		    waypoints.remove(1);
		    PData.setWaypoints(waypoints);
		    Inventory inv = e.getInventory();
		    ItemStack pane = new ItemStack(Material.BLACK_STAINED_GLASS_PANE);
		...
		if (e.getClick() == ClickType.SHIFT_RIGHT && waypoints != null && waypoints.size() > 2) {
		    waypoints.remove(2);
		    PData.setWaypoints(waypoints);
		    Inventory inv = e.getInventory();
		    ItemStack pane = new ItemStack(Material.BLACK_STAINED_GLASS_PANE);
    ...
    public void teleportToWaypoint(Waypoint w) {
	p.closeInventory();
